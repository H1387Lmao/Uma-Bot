"""
Usage (CLI):
    python trainer_card.py \
        --profile    "https://example.com/profile.png" \
        --emoji      "https://example.com/emoji.png" \
        --name       "Spica!" \
        --labels     '{"Referral ID":"888AAA","Club":"Spica!","Fans":"123,456","Carats":"1000","Exp":"67420"}' \
        --comment    "Let's have fun!" \
        --background "https://example.com/bg.png" \
        --output     out.png

Python API:
    from trainer_card import make_card
    make_card(profile_url, emoji_url, name, labels, comment,
              background_url="https://...", output_path="out.png")
"""
...
def _card_mask(w: int, h: int, r: int) -> Image.Image:
    m = Image.new("L", (w, h), 0)
...
def _draw_diamonds(draw: ImageDraw.ImageDraw) -> None:
    s = DIA_HALF
    for col_i, ox in enumerate(range(DIA_START_X, CARD_W + DIA_STEP_X, DIA_STEP_X)):
        for row_i, oy in enumerate(range(BORDER + 10, BAR_Y - DIA_STEP_Y, DIA_STEP_Y)):
            cx = ox
            cy = oy + DIA_STEP_Y // 2
            pts = [
                (cx,     cy - s),
...
def _draw_comment_section(draw: ImageDraw.ImageDraw, comment: str) -> None:
    bx2 = BAR_X + BAR_W
...
def card(
    ...
    _place_photo(card, profile)

    draw = ImageDraw.Draw(card)
    _draw_diamonds(draw)
    _place_header(card, draw, emoji, name)
    _draw_pills(draw, labels)
    _draw_comment_section(draw, comment)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--profile",    required=True, help="URL of character art")
    ap.add_argument("--emoji",      required=True, help="URL of chibi/avatar image")
    ap.add_argument("--name",       default="Trainer", help="Trainer name")
    ap.add_argument("--labels",     required=True,
                    help='JSON dict e.g. \'{"Fans":"123,456"}\'')
    ap.add_argument("--comment",    required=True, help="Comment text")
    ap.add_argument("--background", default=None,
                    help="Optional URL of background image (replaces flat card colour)")
